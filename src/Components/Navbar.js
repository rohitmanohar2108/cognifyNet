...
import {
  ...
  faHeart,
  faNetworkWired,
} from "@fortawesome/free-solid-svg-icons"; // Import faUser for the Profile icon
...
const Navbar = () => {
  ...
  return (
    <nav className="bg-white fixed top-0 w-full z-50 py-1">
      <div className="container mx-auto flex justify-between items-center">
        <h1 className="text-3xl font-bold bg-gradient-to-r from-black to-black bg-clip-text text-transparent tracking-wider text-center">
          CognifyNet
        </h1>
        <ul className="flex space-x-8 relative">
          {[
            ...
            <li key={label} className="flex flex-col items-center cursor-pointer mt-1">
              <Link
                to={to}
                onClick={() => handleItemClick(label.toLowerCase().replace(" ", "-"))}
              ...
              {activeItem === label.toLowerCase().replace(" ", "-") && (
                <div className="border border-black w-full mt-1 rounded-full"></div> 
              )}
            </li>
          ))}
        </ul>
        {/* Profile Photo - Wrap in Link for redirection */}
